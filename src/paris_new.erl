...
create(AppName, Params) ->
  case paris_install:ready() of
    true ->
      ...
      TemplateName = case paris_params:exist("--without-texas", Params) of
        true -> "paris";
        false -> case paris_params:exist("--with-pg", Params) of
            true -> "paris-pg";
            false -> case paris_params:exist("--with-mysql", Params) of
                true -> "paris-mysql";
                false -> "paris-sqlite"
              end
          end
      end,
      case paris_params:exist("--force", Params) or paris_params:exist("-f", Params) of
