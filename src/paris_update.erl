...
run(Params) ->
  {InstallTemplates, InstallPlugins} = case {paris_params:exist("--templates", Params),
                                             paris_params:exist("--plugins", Params)} of
    {false, false} -> {true, true};
    E -> E
  end,
  update(
    paris_params:exist("--master", Params),
    paris_params:exist("--force", Params), 
    InstallTemplates, 
    InstallPlugins
    ).
